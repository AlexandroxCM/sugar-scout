...
export const getSafeBetCarbs = (restaurantName, greenItem) => {
    const lowerGreen = greenItem ? greenItem.toLowerCase() : "";
    ...
    if (restaurant && restaurant.items && restaurant.items.length > 0) {
        // Fuzzy search: find an item that shares significant words, if a real green item is provided
        if (lowerGreen && lowerGreen !== "unknown") {
        ...
        // If no exact match found or it's "Unknown", dynamically return the lowest-carb item from the restaurant data!
        const lowestCarbItem = restaurant.items.reduce((prev, curr) => 
            (prev.stats.carbs < curr.stats.carbs ? prev : curr)
        );
        return { carbs: lowestCarbItem.stats.carbs, velocity: lowestCarbItem.velocity };
    }

    // Comprehensive Research-Based Estimates for all specific "Safe Bet" items purely as a fallback 
    // for restaurants that might not have a detailed items list in nutritionalData
    const specificEstimates = {
        ...
        "Punjab Cafe": { carbs: 4, velocity: "Low" },
        ...
        "Salt & Straw": { carbs: 34, velocity: "High" },
        ...
        "Kitava": { carbs: 24, velocity: "Medium" },
        "The Cheese Steak Shop": { carbs: 31, velocity: "Medium" },
        "Urban Plates": { carbs: 0, velocity: "Low" }
    }
